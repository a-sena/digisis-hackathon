# Hjelp — Oppgave 41: Kvote-allokering (avansert)

Dette er en *bonus*-oppgave med god uttelling. Tre lag å bygge:

1. **Validér input.** `antallBarn ≥ 1` og `dekning ∈ {100, 80}`. Kast
   `IllegalArgumentException` ellers — testene verifiserer det.
2. **Velg base-kvoter.** Avhengig av `dekning`, sett mor/far/felles
   til 18/15/16 (100%) eller 22/19/20 (80%). Når kun én forelder
   har rett, går alt til den ene (49/61 for mor, 40/52 for far).
...
`Kvoter.totalt()` finnes ferdig og hjelper deg å sjekke at sum stemmer
med tabellen i Javadoc.
